...
title: Methods and systems for accessing and controlling a remote desktop of a remote machine in real time by a web browser at a client device via HTTP API utilizing a transcoding server
...
url: http://patft.uspto.gov/netacgi/nph-Parser?Sect1=PTO2&Sect2=HITOFF&p=1&u=%2Fnetahtml%2FPTO%2Fsearch-adv.htm&r=1&f=G&l=50&d=PALL&S1=08589800&OS=08589800&RS=08589800
...
---
The subject technology relates in general to communications and processing and more particularly to methods and systems for accessing and controlling a remote desktop of a remote machine in real time by a web browser at a client device via HTTP API utilizing a transcoding server.
...
In some applications a transcoding server may facilitate accelerate rendering an image of the related remote desktop s at the related web browser by creating and modifying an image of the portion of the remote desktop that is affected by the user s input at the user device.
...
receiving at the transcoding server a user input request from the web browser for accessing or controlling a remote desktop of the remote machine translating at the transcoding server the user input request into an input command compatible with the push protocol 
...
6. The machine readable storage medium of clause 1 further comprising a drawing requests queue configured to forward the drawing request to the HTTP handler in response to a notification from a drawing commands queue that a drawing command is pending for the web browser.
...
26. A processor comprising modules configured to perform the method of any one of the foregoing clauses in paragraphs 0132 0143 .
